...
from transformers import XLNetConfig, AutoConfig, ZMBertModel

from allennlp.data.tokenizers import PretrainedTransformerTokenizer
from allennlp.modules.scalar_mix import ScalarMix
from allennlp.modules.token_embedders.token_embedder import TokenEmbedder
from allennlp.nn.util import batched_index_select, get_device_of

def load_language_mean(language_mean_path, single_layer, language_layer):
    if language_mean_path.endswith(".pkl"):
        language_mean = torch.from_numpy(np.load(language_mean_path, allow_pickle=True)['mean'])
    else:
        language_mean = torch.load(language_mean_path)
    if single_layer:
        return language_mean
    else:
        return language_mean[language_layer]

@TokenEmbedder.register("pretrained_transformer_mds")
class PretrainedTransformerEmbedderMDS(TokenEmbedder):
    """
    ...
    def __init__(
        ...
        *,
        ...
        src_language_mean_path: str = None,
        tgt_language_mean_path: str = None,
        language_layer: int = None,
        ...
        #assert model_name == 'bert-base-multilingual-cased'
        config = AutoConfig.from_pretrained(model_name)
        self.transformer_model = ZMBertModel(config)
        self.src_language_mean = load_language_mean(src_language_mean_path, single_layer, language_layer)
        self.tgt_language_mean = load_language_mean(tgt_language_mean_path, single_layer, language_layer)
    ...
    def forward(
        ...
        if self.language_layer is not None:
            parameters["language_hidden_states"] = (
                self.src_language_mean.to(get_device_of(token_ids)) - self.tgt_language_mean.to(get_device_of(token_ids))
            )
            parameters["language_layer"] = self.language_layer
